...
boot(BootArgs) ->
    ...
    % erlang:display({"init:boot => enter"}),
    % erlang:display({"init:boot => will register self as init"}),

    register(init, self()),


    % erlang:display({"init:boot => will set process_flag"}),

    process_flag(trap_exit, true),

    % erlang:display({"init:boot => will parse boot args "}),

    {Start,Flags,Args} = parse_boot_args(BootArgs),

    % erlang:display({"init:boot => parse boot args done, Start is ", Start, "Flags is", Flags, "Args is ", Args}),
    ...
    % erlang:display({"init:boot => will call boot "}),
...
boot(Start,Flags,Args) ->
    ...
    % erlang:display({"init:boot/3 => enter"}),
    % erlang:display({"init:boot/3 => will start_on_load_handler_process"}),

    start_on_load_handler_process(),

    % erlang:display({"init:boot/3 => will do_boot"}),
    ...
    % erlang:display({"init:boot/3 => will boot_loop"}),
...
boot_loop(BootPid, State) ->
    ...
    % erlang:display({"init:boot_loop => enter"}),

    receive
        MyRec -> 
            % erlang:display({"init:boot_loop => MyRec is ", MyRec}),
            case MyRec of 
...
loop(State) ->
    ...
    % erlang:display({"init:loop => enter the main loop. State is ", State}),
