...
start(Mod, BootArgs) ->
    ...
    % erlang:display({"erl_init:start => enter, Mod is ", Mod}),
    % erlang:display({"erl_init:start => Load the static nifs"}),
    ...
    % erlang:display({"erl_init:start => will call if_loaded", prim_socket}),
...
if_loaded(CondMod, Fun) ->
    ...
    % erlang:display({"erl_init:if_loaded => check loaded ", erlang:loaded() }),
...
if_loaded(CondMod, Fun, [CondMod | _]) ->
    ...
    % erlang:display({"erl_init:if_loaded => found loaded, calling Fun ", Fun }),
